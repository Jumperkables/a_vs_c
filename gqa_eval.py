"""
THIS FILE IS FROM THE OFFICIAL GQA DATASET PAGE. THE EVALUATION DATASET SCRIPT: https://cs.stanford.edu/people/dorarad/gqa/
"""
...
parser.add_argument('--checkpoint_path',required=True,                      type=str,       help = "Path to the checkpoints where the predictions are stored")
parser.add_argument('--score_file_name',required=True, type=str, help = "name of file to save score to")
...
scenes = loadFile(os.path.join(data_dir, args.scenes.format(tier = args.tier)))

# Load questions
print("Loading questions...")
questions = loadFile(os.path.join(data_dir, args.questions.format(tier = args.tier)))

# Load choices
print("Loading choices...")
choices = loadFile(os.path.join(data_dir, args.choices.format(tier = args.tier)))

# Load predictions and turn them into a dictionary
print("Loading predictions...")
checkpoint_path = os.path.join(os.path.dirname(__file__), args.checkpoint_path)
predictions = loadFile(os.path.join(checkpoint_path, args.predictions))
#TODO not needed predictions are in correct format already # predictions = {p["questionId"]: p["prediction"] for p in predictions}
...
attentions = None
if args.grounding:
    with open(os.path.join(checkpoint_path, args.attentions.format(tier = args.tier))) as attentionsFile:
...
def computeGroundingScore(question, sceneGraph, attentionMap):
    ...
    if args.objectFeatures:
        cells = [((x0, y0, x1, y1), attention) for (x0, y0, x1, y1), attention in attentionMap] #cells]
    else:
...
# print
## Save to txt file by redirecting stdout
print("")
f = open(os.path.join(args.checkpoint_path, args.score_file_name), 'w')
sys.stdout = f
for m in metrics:
